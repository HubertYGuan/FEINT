...
class AuthMiddleware(BaseHTTPMiddleware):
  async def dispatch(self, request: Request, call_next: RequestResponseEndpoint):
    if not app.storage.user.get('authenticated', False):
      ...
      if not request.url.path.startswith(('/_nicegui', '/register', '/login', '/auth')):
...
async def login_page():
    ...
    async def attempt_login():
      if not username or not password:
        return
      result = (await call_api(f'/auth/login?username={username.value}&password={password.value}'))
      if result:
        response = auth.Login_Response(**result)
        if response.status == ErrorCode.OTP_REQUIRED or response.status == ErrorCode.INVALID_OTP:
    ...
    ui.label('Password')
    password = ui.input('Password', password=True, password_toggle_button=True).on('keydown.enter', attempt_login)
...
def register_page():
  ...
  async def attempt_register():
    if not username or not password:
      return
    response: str = await call_api(f'/auth/register?username={username.value}&password={password.value}', method='POST')
  ...
  ui.label('Password')
  password = ui.input('Password', password=True, password_toggle_button=True).on('keydown.enter', attempt_register)
...
async def otp_register():
  if app.storage.user.get('otp_enabled', False):
    ...
    ui.image(f'{BACKEND_URL}/auth/otp/generate').style('max-width: 300px; max-height: 300px;')
    ui.label('Once you have scanned the QR code, enter the one-time password from your authenticator app:')
...
async def otp_disable():
  if not app.storage.user.get('otp_enabled', False):
    ...
    ui.label('Please confirm you want to disable OTP by entering the one-time password from your authenticator app:')
    otp = ''
